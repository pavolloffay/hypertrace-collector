...
func (f *jsonFilter) filterJSON(t interface{}, config *PiiElement, key string, actualKey string, jsonPath string, checked bool, filterData *FilterData) (bool, interface{}) {
	switch tt := t.(type) {
	case []interface{}:
		filtered, redacted := f.filterJSONArray(tt, config, key, actualKey, jsonPath, checked, filterData)
		return filtered, redacted
	case map[string]interface{}:
		filtered, redacted := f.filterJSONMap(tt, config, key, actualKey, jsonPath, checked, filterData)
		return filtered, redacted
	case interface{}:
		filtered, redacted := f.filterJSONScalar(tt, config, key, actualKey, jsonPath, checked, filterData)
		return filtered, redacted
	}
